        ...
        extends MIDIMessage
{
    ...
    public static class TimedMessageComparator implements Comparator<MIDITimedMessage>
    {
        public int compare( MIDITimedMessage m1, MIDITimedMessage m2 )
        {
            ...
            try
            {
                // if times are the same, a note off is always larger than a note on,
                // for the same note number.                
                
                if( m1.isNoteOn() && m2.isNoteOff() && m1.getNote()==m2.getNote() )
                {
                    return 1; // m2 is bigger, put it later.
                }
                if( m1.isNoteOff() && m2.isNoteOn() && m1.getNote()==m2.getNote() )
                {
    ...
    public MIDITimedMessage(int time_, int status_, int byte1_, int byte2_ )
    {
